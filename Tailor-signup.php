...
if (isset($_POST['signup'])) {
	...
	if (!$error) {
		if(mysqli_query($conn, "INSERT INTO tailor(user, email, pass) VALUES('" . $name . "', '" . $email . "', '" . md5($password) . "')")) {
			$success_message = "Successfully Registered! <a href='Tailor-login.php'>Click here to Login</a>";
		} else {
...
<head>
<meta charset="utf-8">
<title>Tailor Sign Up</title>
  <meta name="viewport" content="width= device-width, initial-scale=1">
  <link rel="stylesheet" href="tailors-signupcss.css" type="text/css">
</head>
    ...
    <div class="box">
      ...
      <div class="form">
        ...
        <form role="form" autocomplete="off" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post"
          ...
          <div class="form-group">
            <input type="text" autocomplete="off" name="email" placeholder="Enter Email" required
      ...
      <div class="back">
        <button type="Submit" class="btn1">
          <p>Already a user?</p>
          <a href="Tailor-login.php">Login</a>
        </button>
      </div>
    </div>
    </div>
